...
Именно так и делается для того, чтобы анимация графиков работала: радиус окружности задан заранее, а угол меняется, когда двигается слайдер. Запомните эти два соотношения, потому что **это все, что нужно для поворота точки на плоскости на заданный угол**.`
  ...
  },
  {
    ...
    value: (function (md) {
      return (
        md`[Следующая часть *-->*](/demos/3d-graphics/part-2)`
      );
    }),
    node_id: 1264
  },
...
#### Импорт`
  ...
  `
          ...
          range.oninput = () => {
            div.value = range.valueAsNumber;
            view.innerText = format(range.valueAsNumber)
          }
          range.oninput()
...
const nodes = [{
  id: 1231,
  value: 'md`# 3D graphics - first steps - part 1`',
  pinned: false
}, {
  id: 0,
  value: 'md`# Трехмерная графика - первые шаги\n#### Интерактивное пособие для уроков математики и информатики в старших классах средней школы\n---`',
  pinned: false
}, {
  id: 864,
  value: 'md`## Часть 1. Введение`',
  pinned: false
}, {
  id: 197,
  value: 'md`В чем главное преимущество трехмерного изображения по сравнению с плоским, например c фотографией?\nКонечно же в том, что мы можем его **покрутить**, то есть взглянуть на объект под разными углами.\n\nПоэтому, давайте разберемся сначала с **вращением** во всех подробностях.\n\n\n`',
  pinned: false
}, {
  id: 178,
  value: 'md`## Окружность, вращение и угол`',
  pinned: false
}, {
  id: 209,
  value: 'md`Начнем с самого простого: рассмотрим поворот точки на плоскости вокруг заданного центра вращения. Если точка сделает полный оборот и вернется в начальную позицию, получится окружность. Расстояние от точки до центра называют радиусом, и обычно обозначают буквой **R**. Диаметр окружности, обозначается как **D**, и он в 2 раза больше радиуса. Угол отклонения радиуса от оси **x** обычно обозначают греческой буквой **α** (альфа). Участок (сектор) окружности, который по длине равен радиусу отмечен красным цветом. Так как величина угла **α** напрямую **пропорциональна** сектору окружности, угол измеряется в этих секторах. Это международная мера угла так и называется - радиан, и обозначается **rad**`',
  ...
  id: 221,
  value: "viewof alpha1 = slider({label:'Угол:', value:1/3*PI, min:0, max:2*PI, step:0.001, format:(v) => `a = ${Math.round(v / PI * 1000) / 1000 } Pi`})\n",
  pinned: false
}, {
  id: 1146,
  value: 'md`Еще углы измеряются в градусах от 0° до 360°. Но это исторически сложившаяся мера, которая не имеет взаимосвязи с радиусом окружности.\nА математики вообще любят все взаимоувязывать - потому, что так проще запоминать и переходить в расчетах от одной величины к другой. Вот например число π (Pi) показывает сколько длин диаметров понадобится, чтобы нарисовать окружность.`',
  pinned: false
}, {
  id: 182,
  value: 'md`    \n        𝜋 = L / D\n        где L - длина окружности, а D - диаметр`',
  ...
  id: 1197,
  value: 'md`Число Pi можно вычислить лишь приблизительно, потому что оно *иррациональное* (количество знаков после точки бесконечно)`',
  pinned: false
}, {
  id: 65,
  value: 'PI  = Math.PI //-приблизительное значение! число Pi вообще имеет бесконечное число знаков',
  pinned: false
}, {
  id: 1151,
  value: 'md`Длину окружности можно записать как L = π\\\\*D , или если подставить радиус L = π\\\\*2\\\\*R (так как радиус в два раза меньше диаметра). Получается, чтобы начертить окружность, радиус должен "пробежать" угол соответствующий длине окружности в 2π\\\\*R, а угол при этом будет меняться от 0 до 2π (радиан)`',
  pinned: false
}, {
  id: 1226,
  value: 'md`## Нахождение координат точки после поворота на угол`',
  pinned: false
}, {
  id: 340,
  value: 'md`Проще всего считать что центр окружности распологается в начале координат (0,0). Точка, которую мы поворачиваем, будет принимать меняющиеся координаты (x,y). Если соединить отрезками вершины (0,0)-(x,y)-(x,0), то получится прямоугольный треугольник. Радиус R, образованный отрезком (0,0)-(x,y) является **гипотенузой** прямоугольного треугольника.\n`',
  pinned: false
}, {
  id: 348,
  value: "viewof fig2 = {\n  const height = 300\n  const context = DOM.context2d(width, height)\n  const R = height / 2 - 30\n  var x, y, alpha = alpha2\n\n  context.translate(width > 400 ? R + 50 : width / 2, height / 2);\n  context.font = 'bold 14px arial'\n  context.textAlign = 'center'\n  context.strokeStyle = '#ccc'\n  context.beginPath()\n  context.moveTo(0, -height/2)\n  context.lineTo(0, height/2)\n  context.moveTo(-height, 0)\n  context.lineTo(2* R, 0)\n  context.stroke()\n  //\n  context.strokeStyle = '#000'\n  x = R * cos(alpha)\n  y = R * sin(alpha)\n\n  var py = Math.round(alpha / 2 / PI) % 2\n  var px = Math.round(alpha / PI) % 2\n  \n  context.beginPath()\n  context.arc(0, 0, 2, 0, 2 * PI, false)\n  context.stroke()\n\n  context.beginPath()\n  context.arc(x, -y, 2, 0, 2 * PI, false)\n  context.stroke()\n  \n  context.beginPath()\n  context.arc(x, 0, 2, 0, 2 * PI, false)\n  context.stroke()\n  \n  context.beginPath()\n  context.arc(0, 0, 10, 0, 2 * PI-alpha, true)\n  context.fillText('a', 16 * cos(alpha / 2),  -8 * sin(alpha / 2))\n  context.stroke()\n  context.beginPath()\n  context.arc(0, 0, R, 0, 2*PI - alpha, true)\n  context.lineTo(0, 0)\n  context.stroke()\n\n  context.beginPath()\n  context.strokeStyle = '#090'\n  context.rect(x + [-10, 0][px], [-10, 0][py], 10, 10);\n  context.moveTo(0, 0)\n  context.lineTo(x, 0)\n  context.lineTo(x, -y)\n  context.stroke()\n \n  context.beginPath()\n  context.moveTo(x, -y)\n  context.lineTo(0, -y)\n  context.setLineDash([5, 5])\n  context.stroke()\n  context.setLineDash([])\n    \n  context.fillStyle = '#090'\n  context.fillText('x,0', x + 5, [15, -5][py])\n  context.fillText('y', [-10, 10][px], -y + 4)\n\n\n  context.fillStyle = '#000'\n  context.fillText('R', x / 2, -y / 2 - 10)\n  context.fillText('(0,0)', -10, 15)\n  context.fillText('(x,y)', (R + 20) * cos(alpha), -(R + 20) * sin(alpha))\n  return context.canvas\n}",
  ...
  id: 539,
  value: "viewof alpha2 = slider({label:'Угол:', value:1/3*PI, min:0, max:2*PI, step:0.001, format:(v) => `a = ${Math.round(v / PI * 1000) / 1000 } Pi`})",
  pinned: false
}, {
  id: 632,
  value: 'md`Благодаря тому, что один из углов прямоугольного треугольника **никогда не меняется** (он всегда прямой), в его острых углах и катетах заложена пропорциональность соотношений. Например, изменение угла α, заставляет меняться оба катета и противоположный острый угол при неизменном радиусе (гипотенузе). Два основных соотношения это: синус и косинус.\n`',
  pinned: false
}, {
  id: 646,
  value: 'md`Косинус — отношение прилежащего катета к гипотенузе. В нашем случае, прилежащий (тот, который образует угол α) катет - это отрезок длиной в (x)\n\n        cos(α) = x / R\n`',
  pinned: false
}, {
  id: 635,
  value: 'md`Синус — отношение противолежащего (напротив угла α) катета к гипотенузе. В нашем случае это отрезок между точками (x,0) - (x,y), или просто y, потому что длина отрезка всегда равняется (y) и это, видно на графике.\n\n        sin(α) = y / R`',
  pinned: false
}, {
  id: 682,
  value: 'md`Получается, что зная радиус R, и угол α мы всегда можем найти координаты точки (x,y) - как длины сторон прямоугольного треугольника:\n\n        x = R * cos(α)\n        y = R * sin(α)\nИменно так и делается для того, чтобы анимация графиков работала: радиус окружности задан заранее, а угол меняется, когда двигается слайдер. Запомните эти два соотношения, потому что **это все, что нужно для поворота точки на плоскости на заданный угол**.`',
  pinned: false
}, {
  id: 1264,
  value: 'md`[Следующая часть *-->*](3d-graphics-first-steps-part-2)`',
  pinned: false
}, {
  id: 845,
  value: 'md`----\n#### Импорт`',
