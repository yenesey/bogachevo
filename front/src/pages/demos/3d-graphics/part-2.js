...
Тороид, или тор очень удачная фигура для экспериментов. Во-первых его форма - "бублика", намного интереснее, чем просто куб, или пирамида. Во-вторых тор, как вы увидите, весь просто "соткан" из вращений, которые мы прошли в [части 1](part-1).
...
slider({
  ...
  value: 2*PI,
  precision: 3,
  format: v => `Малый круг b = ${Math.round(v / PI * 1000) / 1000 } Pi`,
})
);}),
      node_id: 424
    },
...
slider({
  ...
  value: 0.1,
  precision: 3,
  format: v => `Большой круг a = ${Math.round(v / PI * 1000) / 1000 } Pi`,
})
);}),
      node_id: 402
    },
...
slider({
  ...
  value: 0,
  precision: 3,
  format: v => `Ось x (α = ${Math.round(v / PI * 1000) / 1000 } Pi)`,
})
);}),
      node_id: 47
    },
